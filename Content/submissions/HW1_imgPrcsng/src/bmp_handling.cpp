...
BMPImage * extract_channel_info(BMPImage* input, int mode, int bytes) {
    ...
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            ...
            switch(mode) {
                case 0: output->pixels[i][j] = b; break; // Blue Magnitude
                case 1: output->pixels[i][j] = g; break; // Green Magnitude
                case 2: output->pixels[i][j] = r; break; // Red Magnitude
    ...
    //else{ printf("Please enter the correct number of channels per pixel in th enew image"); return NULL ;}
